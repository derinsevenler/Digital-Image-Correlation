function [validx,validy]=automate_image_rf(grid_x,grid_y,reduction_factor,filenamelist,validx,validy);
...
input_points_x=grid_x/reduction_factor;
base_points_x=grid_x/reduction_factor;

input_points_y=grid_y/reduction_factor;
base_points_y=grid_y/reduction_factor;
...
if resume==1
    ...
    base = uint8(mean(double(imresize(imread(filenamelist(1,:)),1/reduction_factor)),3));            % read in the base image ( which is always  image number one. You might want to change that to improve correlation results in case the light conditions are changing during the experiment

for i=firstimage:(r-1)               % run through all images
    
    
    tic             % start the timer
    input = uint8(mean(double(imresize(imread(filenamelist((i+1),:)),1/reduction_factor)),3));       % read in the image which has to be correlated
...
%     base_points_y=grid_y;
    ...
    imshow(input)                     % update image
    hold on
    plot(grid_x/reduction_factor,grid_y/reduction_factor,'g+')                                % plot start position of raster
