function [validx,validy]=automate_image_prog(grid_x,grid_y,filenamelist,validx,validy)
...
for i=firstimage:(r-1)               % run through all images
    ...
    tic             % start the timer
    base = uint8(mean(double(imread(filenamelist(i,:))),3));            % read in the base image ( which is always  image number one. You might want to change that to improve correlation results in case the light conditions are changing during the experiment
    ...
    if i==1
        validx(:,1)=input_correl_x;                                                     % lets save the data
        savelinex=input_correl_x';
        validy(:,i)=input_correl_y;
        saveliney=input_correl_y';
    else
        validx(:,i) = input_correl_x + validx(:,i-1);
        savelinex = validx(:,i)';
        validy(:,i) = input_correl_y + validy(:,i-1);
        saveliney = validy(:,i);
    end
...
%     title(['# Im.: ', num2str((r-1)),'; Proc. Im. #: ', num2str((i)),'; # Rasterp.:',num2str(row*col), '; Est. Time [h:m:s] ', num2str(h),':',num2str(m),':',num2str(s)])    % plot a title onto the image
    title(['# Im.: ', num2str((r-1)),'; Proc. Im. #: ', num2str((i)),'; # Rasterp.:',num2str(row*col), '; Est. Time [h:m:s] '])    % plot a title onto the image
    drawnow
